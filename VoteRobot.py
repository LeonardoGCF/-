...
def voteOnce(zpid):
    ...
    d, c = h.request('h12729xxtsa.cehehy.top')
  #  conn = httplib2.HTTPConnection("h12729xxtsa.cehehy.top")
    opid = getRandomString(22)
    headers = {'Content-Type': 'application/x-www-form-urlencoded'}
    postParams = urlencode({'zpid': zpid, 'opid': opid, 'md_id': 70, 'act': 'zuopin_toupiao'})
    c.request("POST", "/wtg1/mobile/user.php", postParams, headers)
    c.close()
